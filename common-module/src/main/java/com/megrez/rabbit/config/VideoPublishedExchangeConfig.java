...
public class VideoPublishedExchangeConfig {
    ...
    public FanoutExchange videoPublishedExchange() {
        return new FanoutExchange(VideoPublishedExchange.FANOUT_EXCHANGE_VIDEO_PUBLISHED);
    }

    // 定义视频搜索队列
    @Bean("videoSearchQueue")
    public Queue videoSearchQueue() {
        return new Queue(VideoPublishedExchange.QUEUE_VIDEO_SEARCH, true);
    }

    // 定义通知队列
    @Bean("notificationQueue")
    public Queue notificationQueue() {
        return new Queue(VideoPublishedExchange.QUEUE_NOTIFICATION, true);
    }

    // 定义统计分析队列
    @Bean("analyticsQueue")
    public Queue analyticsQueue() {
        return new Queue(VideoPublishedExchange.QUEUE_ANALYTICS, true);
    }
